...
4. **先验后发（复杂 UI）**：spec 含**表格/图表/嵌套容器（row/col/grid/card/tabs/accordion）任一层级**，或**节点数 ≥2** 时，发出 ```dsh-ui 围栏前先调用 `validate_dsh_ui` 工具（参数 `spec` 传围栏内的 JSON 文本）验证；返回 ❌ 就按错误信息（位置、括号计数、常见原因）修正后重新验证，✅ 再发出；**若 ❌ 回复里附了「已自动修复」的 JSON，直接照抄那份发出，无需再验证**。单个节点的极简 spec 可不验证——但记住：**渲染器的自动修复只兜底标点级与部分结构级错误且不可见，字段名与结构错误唯一可靠的发现途径是 `validate_dsh_ui`**
