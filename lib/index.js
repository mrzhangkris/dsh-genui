...
function solidColor(v) {
	if (typeof v !== "string") return void 0;
	const s = v.trim();
	return s.length <= 64 && SOLID_COLOR_RE.test(s) ? s : void 0;
}
/**
...
function repairItems(list, ctx, depth, path) {
	...
	for (let i = 0; i < list.length; i++) {
		...
		else if (typeof declaredType === "string" && NODE_KEYS[declaredType] !== void 0) ctx.diag?.push({
			kind: "dropped-node",
			path: itemPath,
...
function repairNode(value, ctx, depth, path) {
	...
	if (allowed !== void 0) for (const key of Object.keys(v)) {
		...
		if (!allowed.has(key)) ctx.diag?.push({
			...
			path,
			detail: `${path} 的字段 '${key}' 不是 ${type} 的合法字段，已被无声丢弃（键只取自字段表）`
		});
	}
	switch (type) {
		case "text": {
...
function walkTree(v, cap, depthLeft, ctx) {
	...
	for (const item of v) {
		...
		const children = o !== void 0 && depthLeft > 0 && Array.isArray(o.children) ? walkTree(o.children, cap, depthLeft - 1, ctx) : void 0;
		out.push({
			name,
