...
function repairNode(value, ctx, depth) {
    ...
    switch (type) {
        ...
        case 'slider': {
            ...
            if (label === undefined || min === undefined || max === undefined || value === undefined || min >= max)
                return null;
            return {
        ...
        case 'sort': {
            ...
            if (items === undefined || answer === undefined || items.length === 0 || items.length !== answer.length)
                return null;
...
function repairFormulaSteps(v) {
    ...
    for (const item of v.slice(0, GENUI_LIMITS.maxFormulaSteps)) {
        ...
        if (expression !== undefined)
            out.push({ expression, ...opt('explanation', str(o?.explanation, GENUI_LIMITS.maxString)) });
    }
    return out;
}
...
function repairClassifyGroups(v) {
    ...
    for (const item of v.slice(0, GENUI_LIMITS.maxClassifyGroups)) {
        ...
        if (label !== undefined && items !== undefined)
