 ...
 *   are elided.
 */
import type { GenuiClassify, GenuiFileTreeNode, GenuiFormula, GenuiList, GenuiMatch, GenuiNode, GenuiPlot, GenuiPlotSeries, GenuiScene3D, GenuiSimulation, GenuiSpec } from './spec.ts'
...
function repairNode(value: unknown, ctx: RepairCtx, depth: number): GenuiNode | null {
  ...
  switch (type) {
    ...
    case 'slider': {
      ...
      if (label === undefined || min === undefined || max === undefined || value === undefined || min >= max) return null
      return {
        type: 'slider', label, min, max, value: Math.min(max, Math.max(min, value)),
    ...
    case 'formula': {
      ...
      return { type: 'formula', expression, ...opt('label', str(v.label, 256)), ...opt('steps', repairFormulaSteps(v.steps)) }
    }
    case 'sort': {
      ...
      if (items === undefined || answer === undefined || items.length === 0 || items.length !== answer.length) return null
      return { type: 'sort', items, answer, ...opt('prompt', str(v.prompt, GENUI_LIMITS.maxString)), ...opt('action', str(v.action, 200)) }
    }
    case 'match': {
      const pairs = repairMatchPairs(v.pairs)
      if (pairs === undefined || pairs.length === 0) return null
      return { type: 'match', pairs, ...opt('prompt', str(v.prompt, GENUI_LIMITS.maxString)), ...opt('action', str(v.action, 200)) }
    }
    case 'classify': {
      const groups = repairClassifyGroups(v.groups)
      if (groups === undefined || groups.length === 0) return null
      return { type: 'classify', groups, ...opt('prompt', str(v.prompt, GENUI_LIMITS.maxString)), ...opt('action', str(v.action, 200)) }
    }
    case 'simulation': {
      const steps = repairSimulationSteps(v.steps)
      if (steps === undefined || steps.length === 0) return null
      return {
        type: 'simulation', steps,
        ...opt('title', str(v.title, 256)),
        ...opt('current', int(v.current, 0, steps.length - 1)),
        ...opt('intervalMs', int(v.intervalMs, 250, 60_000)),
...
function repairFormulaSteps(v: unknown): GenuiFormula['steps'] | undefined {
  if (!Array.isArray(v)) return undefined
  const out: NonNullable<GenuiFormula['steps']> = []
  for (const item of v.slice(0, GENUI_LIMITS.maxFormulaSteps)) {
    const o = obj(item)
    const expression = o === undefined ? undefined : str(o.expression, GENUI_LIMITS.maxString)
    if (expression !== undefined) out.push({ expression, ...opt('explanation', str(o?.explanation, GENUI_LIMITS.maxString)) })
  }
  return out
}

function repairMatchPairs(v: unknown): GenuiMatch['pairs'] | undefined {
  if (!Array.isArray(v)) return undefined
  const out: GenuiMatch['pairs'] = []
  for (const item of v.slice(0, GENUI_LIMITS.maxPracticeItems)) {
    const o = obj(item)
    const left = o === undefined ? undefined : str(o.left, 512)
    const right = o === undefined ? undefined : str(o.right, 512)
    if (left !== undefined && right !== undefined) out.push({ left, right })
  }
  return out
}

function repairClassifyGroups(v: unknown): GenuiClassify['groups'] | undefined {
  if (!Array.isArray(v)) return undefined
  const out: GenuiClassify['groups'] = []
  for (const item of v.slice(0, GENUI_LIMITS.maxClassifyGroups)) {
    const o = obj(item)
    const label = o === undefined ? undefined : str(o.label, 256)
    const items = o === undefined ? undefined : repairStrings(o.items, GENUI_LIMITS.maxPracticeItems, 512)
    if (label !== undefined && items !== undefined) out.push({ label, items })
  }
  return out
}

function repairSimulationSteps(v: unknown): GenuiSimulation['steps'] | undefined {
  if (!Array.isArray(v)) return undefined
  const out: GenuiSimulation['steps'] = []
  for (const item of v.slice(0, GENUI_LIMITS.maxSimulationSteps)) {
    const o = obj(item)
    const label = o === undefined ? undefined : str(o.label, 256)
    const content = o === undefined ? undefined : str(o.content, GENUI_LIMITS.maxString)
    if (label !== undefined && content !== undefined) out.push({ label, content })
  }
  return out
}
